...
export function patchAttrs(element, oldAttrs, newAttrs) {
  ...
  allKeys.forEach(key => {
    ...
    // Skip if values are identical (except for special cases)
    if (oldValue === newValue && key !== 'value' && key !== 'checked' && !key.startsWith('on')) {
      return;
    }

    if (key === 'style' && typeof newValue === 'object' && newValue !== null) {
      // Optimized style handling - only update changed properties
      const oldStyle = (typeof oldValue === 'object' && oldValue !== null) ? oldValue : {};
      const newStyle = newValue;

      // Check all keys from both old and new styles
      const allStyleKeys = new Set([...Object.keys(oldStyle), ...Object.keys(newStyle)]);
      allStyleKeys.forEach(prop => {
        const oldProp = oldStyle[prop];
        const newProp = newStyle[prop];
        if (oldProp !== newProp) {
          if (newProp === undefined || newProp === null || newProp === '') {
            element.style[prop] = '';
          } else {
            element.style[prop] = newProp;
          }
        }
      });
